    ...
    <div class="font-serif bg-gray-900 lg:flex p-3 my-20 mx-10 rounded-md text-white items-center justify-center w-full ">
      ...
      <div class="text-left">
        <ul v-for="profile in profile" :key="profile.id">
          ...
          <li v-if="isPhone">
            Phone Number:
            <input type="number" id="phone" name="phone" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" v-model="phone" required placeholder="Enter your new Phone number"
              class="w-full py-2 text-black px-1 rounded-md" @keyup.enter="updatePhone()" />
              <label>Please enter a number correct phone number</label>
...
export default {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:5000/studentInfo/id/${this.$route.params.id}`).then((response) => {
  ...
  methods: {
    ...
    async updatePassword() {
      ...
      if (this.password == '') {
        ...
        try {
          await axios.put(`http://localhost:5000/updateStudentProfile/${this.$route.params.id}`, {
    ...
    async updatePhone() {
      ...
      if (this.phone == '') {
        ...
        try {
          await axios.put(`http://localhost:5000/updateStudentUserProfile/${this.$route.params.id}`, {
            // username: this.username,
            type: "phone_number",
            value: this.phone
          })
