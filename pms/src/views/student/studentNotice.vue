<template>
  <div class="mx-4 pt-4 font-serif">
        ...
        <tr class="border-b" v-for="item in this.notices " :key="item.id">
          ...
          <td class="text-sm text-blue-500 font-light whitespace-nowrap ">
            <RouterLink class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500"
              :to="{ name: 'studentNoticeView', params: { id: this.$route.params.id, group: this.group, notice: item.id } }">
              See
              More
            </RouterLink>
          </td>
...
export default {
  ...
  async mounted() {
    ...
    console.log(faculty);
    await axios.get(`http://localhost:5000/notice_detail/${this.$route.params.id}/${faculty}/student/all`)
      .then(response => {
