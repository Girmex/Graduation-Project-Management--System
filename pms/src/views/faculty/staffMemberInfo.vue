    ...
    <div class="text-left">
      <ul v-for="profile in profile" :key="profile.id">
        ...
        <li>Position: {{ profile.role_name }}</li>
          <li v-if="isPassword && profile.position == 7 || isPassword && profile.position==5 || isPassword && profile.position==4 || isPassword && profile.position==3">
            <select v-model="position" name="dropdown" class="w-full border text-black rounded py-1 my-2">
         ...
         <li v-if="isPassword && profile.position == 7 || isPassword && profile.position==5 || isPassword && profile.position==4 || isPassword && profile.position==3"><button @click="updateRoll()"  class="bg-blue-600 text-white py-1 w-full text-center rounded-sm hover:bg-blue-900 ">Save</button></li> 
...
export default {
  ...
  methods: {
    async updateRoll() {
      ...
      let chair_id=localStorage.getItem('chair_id');
      console.log(chair_id)
      try {
        await axios.put(`http://localhost:5000/up/${this.$route.params.id}/${chair_id}`, {
