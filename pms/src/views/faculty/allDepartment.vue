       ...
       <div class="m-4">
           ...
           <div :class="divStyle">
            ...
            <div class="w-1/4 right-4 bg-orange-500 hover:bg-blue-700 active:bg-slate-300 px-4 py-2 border rounded-md text-white">
                    <router-link :to="{ name: 'addDepartment',params:{facultyId:this.$route.params.facultyId} }">Add New Department</router-link>
            </div></div>
                      ...
                      <tbody>
                        <tr class="border-b text-white"  v-for="item in academic " :key="item.department_id">
                          ...
                          <td class="text-sm text-blue-500 font-light whitespace-nowrap ">
                           <span class="mr-12 p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500"><router-link :to="{name:'departmentInfo',params:{id:item.department_id}}" >See More</router-link></span> 
                           <span class="mr-12 p-1 rounded-md bg-red-700 text-white hover:bg-red-500"><button @click="deleteDepartment(item.department_id)">Delete</button></span>
                          </td>
                         </tr>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(`http://localhost:5000/DepDepartmentListInfo/faculty_id/${this.$route.params.facultyId}`)
      .then((response) => {
