<template>
    ...
    <div v-if="this.names.length!==0" class="w-full mx-2">
    ...
    export default{
      ...
      if(!token){
       this.$router.push({name:'Login'})
             ...
             axios.get(`http://localhost:5000/noticeList/1/${this.$route.params.facultyId}/''/''`)
                   .then(response => {
            ...
            axios.get(`http://localhost:5000/listLength/notice/faculty_id/${this.$route.params.facultyId}/''/''`)
             .then(response => {
  ...
  return new Date(date).toLocaleString('en-US',options)
        ...
        async  getPage(page){
             ...
             await axios.get(`http://localhost:5000/noticeList/${this.page}/${this.$route.params.facultyId}/''/''`)
                   .then(response => {
                      this.names = response.data[0]; 
                      console.log(this.pages)
                        })
        ...
        async  backPage(page){
          ...
          this.page=1
         await axios.get(`http://localhost:5000/List/${this.page}/${this.$route.params.facultyId}/''/''`)
          .then(response => {
            this.names = response.data[0];
        ...
        async  fetchData(){
            await axios.get(`http://localhost:5000/search/notice/faculty_id/${this.$route.params.facultyId}/''/''/title/date/${this.search}`)
             .then(response => {
             this.names=response.data[0];
               console.log(this.pages)
             })
