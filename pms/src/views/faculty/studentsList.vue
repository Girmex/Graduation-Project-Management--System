        ...
        <tbody>
          <tr class="border-b" v-for="item in this.names " :key="item.id">
            ...
            <td class="text-sm  font-light whitespace-nowrap ">
              <router-link class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500"
                :to="{ name: 'studentsInfo', params: { id: item.id } }">See More</router-link>
            </td>
          </tr>
...
export default {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:5000/ListStudent/1/students/students.faculty_id/${this.$route.params.facultyId}/${this.$route.params.type}/${this.$route.params.value}/first_name`)
      .then(response => {
    ...
    axios.get(`http://localhost:5000/listLength/students/students.faculty_id/${this.$route.params.facultyId}/${this.$route.params.type}/${this.$route.params.value}`)
      .then(response => {
  ...
  methods: {
    async getPage(page) {
      ...
      await axios.get(`http://localhost:5000/ListStudent/${this.page}/students/students.faculty_id/${this.$route.params.facultyId}/${this.$route.params.type}/${this.$route.params.value}/first_name`)
        .then(response => {
          this.names = response.data[1];
          console.log(this.pages)
        })
    ...
    async fetchData() {
      await axios.get(`http://localhost:5000/searchStudent/students/students.faculty_id/${this.$route.params.facultyId}/${this.$route.params.type}/${this.$route.params.value}/first_name/second_name/${this.search}`)
        .then(response => {
          this.names = response.data[1];
          console.log(this.pages)
        })
