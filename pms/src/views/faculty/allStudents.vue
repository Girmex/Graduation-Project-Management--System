                      ...
                      <tbody>
                        <tr class="border-b"  v-for="item in names " :key="item.id">
                          ...
                          <td class="text-sm font-light whitespace-nowrap ">
                            <router-link class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'studentsInfo',params:{id:item.id,department_id:item.department_id,chair_id:item.chair_id,roll:item.position}}" >See More</router-link>
                          </td>
                         </tr>
    ...
    export default{
      ...
      if(!token){
       this.$router.push({name:'Login'})
             ...
             axios.get(`http://localhost:5000/ListStudent/1/students/students.faculty_id/${this.$route.params.facultyId}/''/''/first_name`)
                   .then(response => {
            ...
            axios.get(`http://localhost:5000/listLength/students/students.faculty_id/${this.$route.params.facultyId}/''/''`)
             .then(response => {
        ...
        async  getPage(page){
             ...
             await axios.get(`http://localhost:5000/ListStudent/${this.page}/students/students.faculty_id/${this.$route.params.facultyId}/''/''/first_name`)
                   .then(response => {
                      this.names = response.data[0]; 
                      console.log(this.pages)
                        })
        ...
        async  fetchData(){
            await axios.get(`http://localhost:5000/searchStudent/students/students.faculty_id/${this.$route.params.facultyId}/faculty_id/${this.$route.params.facultyId}/first_name/second_name/${this.search}`)
             .then(response => {
             this.names=response.data[0];
               console.log(this.pages)
             })
