<template>
    <div class="w-full flex font-serif">
      ...
      <div v-if="this.names.length !== 0" class="w-full px-4">
        <div class="flex justify-start lg:justify-end lg:px-3 py-1"><input @keyup.enter="fetchData" type="text"
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        await axios.get(`http://localhost:5000/searchProject/allproject/best_project/yes/faculty_id/1/title/advisor/${this.search}`)
          .then(response => {
            this.names = response.data[0];
            console.log(this.pages)
          })
