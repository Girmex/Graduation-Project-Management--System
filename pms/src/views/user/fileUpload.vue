      ...
      this.success=false
        ...
        try{
          if(this.$route.params.status=='Created'){
             axiosInstance.post(`uploadProposal/${this.$route.params.id}/proposal`,formData);
        
          }
          else{
     axiosInstance.post(`upload/${this.$route.params.id}/final`,formData)
      .then(response => {
      ...
      cancel(){
         axiosInstance.get(`remove/${this.$route.params.id}/final/${this.file.name}`)
         .then(response=>{
