<template>
          ...
          <tbody>
            <tr class="border-b text-black" v-for="item in this.names " :key="item.id">
              ...
              <td class="text-sm font-light whitespace-nowrap ">
                <RouterLink class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'chairProjectInfo',params:{chairId:this.$route.params.chairId,id:item.id}}">See More</RouterLink>
              </td>
            </tr>
  ...
  export default {
    ...
    mounted() {
      ...
      axios.get(`http://localhost:5000/ListProject/1/allproject/chair_id/${this.$route.params.chairId}/best_project/yes/title`)
        .then(response => {
      ...
      this.page = 1
      axios.get(`http://localhost:5000/listLength/allproject/chair_id/${this.$route.params.chairId}/best_project/yes`)
           .then(response => {
    ...
    methods: {
      async getPage(page) {
        ...
        await axios.get(`http://localhost:5000/ListProject/${this.page}/allproject/chair_id/${this.$route.params.chairId}/best_project/yes/id`)
          .then(response => {
            this.names = response.data[1];
            console.log(this.pages)
          })
      ...
      async fetchData() {
        await axios.get(`http://localhost:5000/searchProject/allproject/chair_id/${this.$route.params.chairId}/best_project/yes/title/advisor/${this.search}`)
          .then(response => {
            this.names = response.data[1];
            console.log(this.pages)
          })
