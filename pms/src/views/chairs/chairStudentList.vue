                      ...
                      <tbody>
                        <tr class="border-b"  v-for="item in this.names " :key="item.names_id">
                          ...
                          <td class="text-sm text-blue-500 font-light whitespace-nowrap ">
                            <router-link class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'chairStudentInfo',params:{id:item.id}}" >See More</router-link>
                            <button  @click="deleteStudent(item.id)" class="ml-9 p-1 rounded-md bg-red-700 text-white hover:bg-red-500">delete</button>
    ...
    export default{
      ...
      if(!token){
       this.$router.push({name:'Login'})
             ...
             axios.get(`http://localhost:5000/List/1/students/chair_id/${this.$route.params.chairId}/${this.$route.params.type}/${this.$route.params.value}/first_name`)
                   .then(response => {
            ...
            axios.get(`http://localhost:5000/listLength/students/chair_id/${this.$route.params.chairId}/${this.$route.params.type}/${this.$route.params.value}`)
             .then(response => {
        ...
        async  getPage(page){
             ...
             await axios.get(`http://localhost:5000/List/${this.page}/students/chair_id/${this.$route.params.chairId}/${this.$route.params.type}/${this.$route.params.value}/first_name`)
                   .then(response => {
                      this.names = response.data[1]; 
                      console.log(this.pages)
                        })
        ...
        async  fetchData(){
            await axios.get(`http://localhost:5000/search/students/chair_id/${this.$route.params.chairId}/${this.$route.params.type}/${this.$route.params.value}/first_name/second_name/${this.search}`)
             .then(response => {
             this.names=response.data[1];
               console.log(this.pages)
             })
