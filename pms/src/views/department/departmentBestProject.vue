<template>
    <div class="w-full flex font-serif">
      ...
      <div v-if="this.names.length !== 0" class="w-full px-4">
        <div class="flex justify-end px-3 py-1"><input @keyup.enter="fetchData" type="text"
            class="bg-gray-800 rounded-l-md py-1 text-white" v-model="search" placeholder="search here...">
        ...
        <table class="table-auto w-full table-responsive ">
          ...
          <tbody>
            <tr class="border-b text-black" v-for="item in this.names " :key="item.id">
              ...
              <td class="text-sm font-light whitespace-nowrap ">
                <RouterLink class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'departmentProjectInfo',params:{departmentId:this.$route.params.departmentId,id:item.id}}">See More</RouterLink>
              </td>
            </tr>
  ...
  export default {
    ...
    mounted() {
      ...
      axios.get(`http://localhost:5000/ListProject/1/allproject/department_id/${this.$route.params.departmentId}/best_project/yes/title`)
        .then(response => {
      ...
      this.page = 1
      axios.get(`http://localhost:5000/listLength/allproject/department_id/${this.$route.params.departmentId}/best_project/yes`)
           .then(response => {
    ...
    methods: {
      async getPage(page) {
        ...
        await axios.get(`http://localhost:5000/ListProject/${this.page}/allproject/department_id/${this.$route.params.departmentId}/best_project/yes/id`)
          .then(response => {
            this.names = response.data[1];
            console.log(this.pages)
          })
      ...
      async fetchData() {
        await axios.get(`http://localhost:5000/searchProject/allproject/department_id/${this.$route.params.departmentId}/best_project/yes/title/advisor/${this.search}`)
          .then(response => {
            this.names = response.data[1];
            console.log(this.pages)
          })
