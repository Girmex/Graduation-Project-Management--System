<template>
    <div class="px-4 pt-4 font-serif ">
                    ...
                    <tbody>
                      <tr class="border-b"  v-for="item in this.notices " :key="item.id">
                        ...
                        <td class="text-sm text-blue-500 font-light whitespace-nowrap ">
                          <RouterLink class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500"  :to="{name:'departmentNoticeView',params:{departmentId:this.$route.params.departmentId,notice:item.id}}">See More</RouterLink> 
                        </td>
                       </tr>
    ...
    let token=localStorage.getItem('token')
      if(!token){
       this.$router.push({name:'Login'})
        ...
        console.log(faculty);
       await axios.get(`http://localhost:5000/notice_detail/${this.$route.params.departmentId}/${faculty}/department/all`)
          .then(response => {
