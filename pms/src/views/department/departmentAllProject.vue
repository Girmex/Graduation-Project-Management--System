    ...
    <div class="flex justify-end px-3 py-1"><input @keyup.enter="fetchData" type="text" class="bg-gray-800 text-white rounded-l-md py-1" v-model="search" placeholder="search here...">
      ...
      <div class="overflow-x-auto ">
        <table class="table-auto w-full table-responsive ">
                    ...
                    <tbody>
                      <tr class="border-b border-gray-400 "  v-for="item in this.names " :key="item.id">
  ...
  export default{
      ...
      if(!token){
       this.$router.push({name:'Login'})
           ...
           axios.get(`http://localhost:5000/List/1/allproject/department_id/${this.$route.params.departmentId}/''/''/id`)
                 .then(response => {
          ...
          axios.get(`http://localhost:5000/listLength/allproject/department_id/${this.$route.params.departmentId}/''/''`)
           .then(response => {
      ...
      async  getPage(page){
           ...
           await axios.get(`http://localhost:5000/List/${this.page}/allproject/department_id/${this.$route.params.departmentId}/''/''/id`)
                 .then(response => {
                    this.names = response.data[0]; 
                    console.log(this.pages)
                      })
      ...
      async  fetchData(){
        await axios.get(`http://localhost:5000/searchProject/allproject/department_id/${this.$route.params.departmentId}/department_id/1/title/advisor/${this.search}`)
           .then(response => {
           this.names=response.data[0];
