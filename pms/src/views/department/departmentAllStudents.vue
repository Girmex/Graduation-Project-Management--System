                      ...
                      <tbody>
                        <tr class="border-b"  v-for="item in names " :key="item.id">
                          ...
                          <td class="text-sm text-blue-500 font-light whitespace-nowrap ">
                            <router-link class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'departmentStudentInfo',params:{id:item.id}}" >See More</router-link>
                            <span class="ml-9"><button @click="deleteStudent(item.id)" class="p-1 rounded-md bg-red-700 text-white hover:bg-red-500">delete</button></span>
                          </td>
                         </tr>
    ...
    export default{
      ...
      if(!token){
       this.$router.push({name:'Login'})
             ...
             axios.get(`http://localhost:5000/List/1/students/department_id/${this.$route.params.departmentId}/''/1/first_name`)
                   .then(response => {
            ...
            axios.get(`http://localhost:5000/listLength/students/department_id/${this.$route.params.departmentId}/''/1`)
             .then(response => {
        ...
        async  getPage(page){
             ...
             await axios.get(`http://localhost:5000/List/${this.page}/students/department_id/${this.$route.params.departmentId}/''/1/first_name`)
                   .then(response => {
                      this.names = response.data[0]; 
                      console.log(this.pages)
                        })
        ...
        async  backPage(page){
              ...
              if(this.page < (this.pages +1) && this.page>1){
               ...
               await axios.get(`http://localhost:5000/List/${this.page}/students/department_id/${this.$route.params.departmentId}/''/''/first_name`)
                .then(response => {
                  this.names = response.data[0];
        ...
        async  fetchData(){
            await axios.get(`http://localhost:5000/search/students/department_id/${this.$route.params.departmentId}/''/''/first_name/id/${this.search}`)
             .then(response => {
             this.names=response.data[0];
               console.log(this.pages)
             })
