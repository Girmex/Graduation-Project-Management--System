<template>
    ...
    <div class="w-full px-4">
      <div class="flex justify-start lg:justify-end px-3 py-1"><input @keyup.enter="fetchData" type="text" class="bg-gray-800 text-white rounded-l-md py-1" v-model="search" placeholder="search here...">
        ...
        <table class="table-auto w-full table-responsive ">
                      <thead class="border-b">
                        <tr>
                          ...
                          <th scope="col" class="text-sm text-center font-medium text-gray-900   bg-gray-300">
                          Action
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr class="border-b"  v-for="item in this.names " :key="item.id">
                          ...
                          <td class="text-sm text-center font-light whitespace-nowrap ">
                             <RouterLink class="p-1 rounded-md bg-blue-700 text-white hover:bg-blue-500" :to="{name:'advisorProjectDetail',params:{id:this.$route.params.id,group:item.id}}">See More</RouterLink>
                          </td>
                         </tr>
    ...
    export default{
      ...
      this.departmentId=localStorage.getItem('department_id')
             axios.get(`http://localhost:5000/List/1/allproject/department_id/${this.departmentId}/status/Uploaded/id`)
                   .then(response => {
            ...
            axios.get(`http://localhost:5000/listLength/allproject/department_id/${this.departmentId}/status/Uploaded`)
             .then(response => {
        ...
        async  getPage(page){
             ...
             await axios.get(`http://localhost:5000/List/${this.page}/allproject/department_id/${this.departmentId}/status/Uploaded/id`)
                   .then(response => {
                      this.names = response.data[1]; 
                      console.log(this.pages)
                        })
        ...
        async  fetchData(){
          await axios.get(`http://localhost:5000/searchProject/allproject/department_id/${this.departmentId}/status/Uploaded/title/advisor/${this.search}`)
             .then(response => {
             this.names=response.data[1];
