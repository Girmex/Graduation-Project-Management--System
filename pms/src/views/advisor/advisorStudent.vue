...
<template>
  <div class="grid grid-cols-1 m-4 lg:flex lg:flex-row-reverse">
    ...
    <div :class="divStyle" >
          ...
          <ul class="text-left font-serif  pt-2">
              ...
              <li  v-for="item in this.academic " :key="item.id"  ><span></span> <RouterLink class="flex lg:pl-3 py-2 focus:bg-white focus:text-black shadow-md shadow-gray-600 hover:bg-slate-400 hover:text-black" :to="{name:'advisorStudentList',params:{id:this.$route.params.id,group:item.id}}">{{ item.title }}</RouterLink>
              </li>
                                  
          </ul>
      </div>
      <RouterView class="w-full font-serif" :key="$route.path"></RouterView>
    </div>
